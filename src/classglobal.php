	...
	<body>
		<div id = "base">
			...
			<table>				
				...
				$niveau= $_POST['choix'];
				// On récupère nos pseudo, nombre de coup et duree de notre partie pour le classement global à travers une requête SQL, seul 10 joueur sont selectionnées  
				$classgloba = oci_parse($dbConn,'SELECT pseudo,nb_coup,duree FROM "21602883".vue_highscore_global WHERE id_niveau=:niv and rownum <= 10 order by rownum');
				...
				while(oci_fetch($classgloba)){ 
					$pseudo = oci_result($classgloba, 1); // récupèrer le pseudo 
					$nbcoup = oci_result($classgloba, 2); // récupèrer le nombre coup
					$duree = oci_result($classgloba, 3); // récupèrer la durée
					print "<tr>";
