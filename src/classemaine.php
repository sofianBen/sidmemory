		...
		<div id = "base">
			...
			<table>
				...
				while(oci_fetch($classemai)){ // Pour chaque ligne de notre requête on:
					$pseudo = oci_result($classemai, 1); // récupère le pseudo de notre requête SQL
					$nbcoup = oci_result($classemai, 2); // récupère le nb coup de notre requête SQL
					$duree = oci_result($classemai, 3); // récupère la durée de notre requête SQL
