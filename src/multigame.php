...
$reqpartie = oci_parse($dbConn, 'begin "21602883".creation_partie_multi(:pId_niveau, :pId_joueur1, :pId_joueur2, :rId_partie); end;');
...
$requetepseudoj1 =  oci_parse($dbConn,'begin :r := "21602883".id_joueur_en_pseudo(:id); end;'); // requête SQL qui fait appele à la fonction id_joueur_en_pseudo qui permet d'obtenir le pseudo du joueur1
oci_bind_by_name($requetepseudoj1, ':id', $id_j1,10);
oci_bind_by_name($requetepseudoj1, ':r', $pseudoj1,10);
oci_execute($requetepseudoj1);

$requetepseudoj2 =  oci_parse($dbConn,'begin :r := "21602883".id_joueur_en_pseudo(:id); end;'); // requête SQL qui fait appele à la fonction id_joueur_en_pseudo qui permet d'obtenir le pseudo du joueur2
		...
		<div id = "base">
			...
			for ($i=1; $i<=$LigneMax; $i++) { 
				...
				for ($j=1; $j<= $ColonneMax; $j++) { 
					...
					$reqidcarte =  oci_parse($dbConn,'SELECT id_carte FROM "21602883".CARTE where ligne = :li and colonne = :co and id_partie = :pa');
